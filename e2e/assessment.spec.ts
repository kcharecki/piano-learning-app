 ...
 * button that opens a modal `<dialog>` — the dialog blocks the rest of the
 * page (native `showModal()`), so this closes it again once the new score's
 * title is confirmed on screen, before returning control to the caller.
 */
...
/**
 * UI-04b: the MIDI status line moved out of every screen's own content flow
 * into the shell topbar's input-status chip, whose popover holds the detail
 * — open it, check the text, then Escape closes it again (same contract
 * e2e/input-capability-banner.spec.ts already proves for the chip itself).
 */
