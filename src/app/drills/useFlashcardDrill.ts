 ...
 * (`useFlashcardStore`), asks `nextCard` what to show, times the answer with
 * the injected `Clock`, and writes the graded card back.
 *
...
export function useFlashcardDrill(options: UseFlashcardDrillOptions): UseFlashcardDrill {
  ...
  useEffect(() => {
    const now = clock.now()
    setCurrent(nextStaffToKeyCard(deck, cards, now, rng))
  ...
  function answerNote(note: Midi): void {
    ...
    const now = clock.now()
    const elapsedMs = now - promptShownAtRef.current
    const result = gradeAnswer(card, { midi: note }, elapsedMs)
    const existing = cardsById[card.id] ?? newCard(card.id, now)
    const updated = review(existing, result.grade, now, rng)
    upsertCard(updated)
    setLastGrade(result)
    ...
    const updatedCards = cards.some((c) => c.id === updated.id)
      ...
      : [...cards, updated]
    setCurrent(nextStaffToKeyCard(deck, updatedCards, now, rng))
    promptShownAtRef.current = now
  }
  ...
  const stats = useMemo(() => {
    ...
    return retentionStats(deckCards, clock.now())
  }, [deck, cards, clock])
