...
import {
  ...
  triadSequenceNotes,
  type TriadForm,
} from '@core/technique/triadSequence.ts'
 ...
 * Preparatory left hand plays in. The syllabus engraves the pattern once, on a
 * single treble-clef staff, marked "hands separately", which fixes the notes
 * and leaves the register to the player; the left hand therefore takes it an
 * octave down, where it sits inside the bass staff. This is NOT `handTonic`'s
 * two-octave hands-together offset: these drills are hands separately only.
 */
const TRIAD_SEQUENCE_LEFT_OCTAVE_OFFSET = -1

function triadSequenceScore(
  id: string,
  ...
  const start =
    hand === 'left'
      ? spell(tonic.letter, tonic.alter, tonic.octave + TRIAD_SEQUENCE_LEFT_OCTAVE_OFFSET)
      : tonic
  const notes = triadSequenceNotes(start, scaleType, hand, form)
  const measureCount = Math.ceil(triadSequenceDurationTicks(form) / WHOLE)
  const measures = Array.from({ length: measureCount }, (_, i) => (i === 0 ? { keyFifths } : {}))
  return makeScore({
    id,
    meta: { title: drill.title },
    measures,
    notes: [...notes],
    tempos: [{ tick: 0, bpm }],
  })
  ...
  switch (drill.kind) {
    ...
    case 'triad-sequence': {
      ...
      return triadSequenceScore(
        ...
        drill.form,
        bpm,
        keyFifthsFor(drill.tonic, qualityOf(type)),
      )
    }
  }
}
 ...
 * scale, ascending one octave, hands separately, in two forms — broken
 * (♩=60) and solid/blocked (♩=72). The notes themselves are built in
 * `triadSequence.ts`; this only names and grades the drill.
 */
...
const ALL_DRILLS: readonly TechniqueDrill[] = [
  ...
  // both forms at the syllabus's own tempi.
  ...(
    [
      ['broken', 60],
      ['solid', 72],
