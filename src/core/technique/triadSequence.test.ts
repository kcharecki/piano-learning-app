...
describe('triadSequenceScore', () => {
  const scoreFor = (hand: Hand, form: TriadForm) =>
    triadSequenceScore({
      id: `triad-${hand}-${form}`,
      title: `C major triad sequence, ${form}`,
      tonic: spell('C', 0, 4),
      scaleType: 'major',
      hand,
      form,
  ...
  it('lays the broken form out in exactly the bars its ticks need', () => {
    const score = scoreFor('right', 'broken')
    expect(score.measures).toHaveLength(triadSequenceDurationTicks('broken') / WHOLE)
    expect(score.measures).toHaveLength(2)
    // Nothing spills past the last barline: the drill ends where the bar does.
    const end = Math.max(...score.notes.map((n) => n.startTick + n.durationTicks))
    expect(end).toBe(triadSequenceDurationTicks('broken'))
  })

  it('lays the solid form out in four bars, the trailing rests included', () => {
    const score = scoreFor('right', 'solid')
    expect(score.measures).toHaveLength(4)
    // The last block is a quarter followed by its quarter rest, so the notes
    // stop a beat before the final barline — the rest is the engraver's job.
    const end = Math.max(...score.notes.map((n) => n.startTick + n.durationTicks))
    expect(end).toBe(triadSequenceDurationTicks('solid') - QUARTER)
  })

  it('drops the left hand an octave so it stays inside the bass staff', () => {
    const right = scoreFor('right', 'broken')
    const left = scoreFor('left', 'broken')
    expect(left.notes.map((n) => n.midi)).toEqual(right.notes.map((n) => n.midi - 12))
    // The point of the drop: the top of the left-hand sequence stays at or
    // below middle C's octave rather than climbing a fifth above it.
    expect(Math.max(...left.notes.map((n) => n.midi))).toBeLessThan(
      Math.min(...right.notes.map((n) => n.midi)) + 12,
    )
  })

  it('carries the drill title, the key signature and the requested tempo', () => {
    const score = triadSequenceScore({
      id: 'triad-g',
      title: 'G major triad sequence, broken, right hand',
      tonic: spell('G', 0, 4),
      scaleType: 'major',
      hand: 'right',
      form: 'broken',
    ...
    expect(score.meta.title).toBe('G major triad sequence, broken, right hand')
    expect(score.measures[0]?.keyFifths).toBe(1)
    expect(score.tempos).toEqual([{ tick: 0, bpm: 66 }])
  })

  it('keeps the tuplet on every broken note through score construction', () => {
    // `makeScore` rebuilds every note; a field it forgets to copy would be
    // dropped silently here and the engraving would lose all eight numerals.
    const score = scoreFor('right', 'broken')
    expect(score.notes.every((n) => n.tuplet?.actual === 3 && n.tuplet.normal === 2)).toBe(true)
    expect(score.notes.filter((n) => n.tuplet?.position === 'start')).toHaveLength(
      TRIAD_SEQUENCE_STEPS,
    )
    expect(score.notes.filter((n) => n.tuplet?.position === 'stop')).toHaveLength(
      TRIAD_SEQUENCE_STEPS,
    )
  })

  it('leaves the solid form free of tuplets', () => {
    expect(scoreFor('right', 'solid').notes.every((n) => n.tuplet === undefined)).toBe(true)
  })
})
