...
export const NO_NOTES: readonly ScoreNote[] = []
...
export function maxNoteDurationTicks(notes: readonly ScoreNote[]): Ticks {
  let max = 0
...
/**
 * Index of the measure containing `tick`, or -1. A binary search, which the
 * model's "measures are contiguous from tick 0" rule is what makes valid.
 *
 * Lives here rather than in `scoreQueries.ts` because `buildNotes` needs it
 * while it is still assembling a Score, and a query module importing back into
 * the model would be a cycle.
 */
export function measureIndexAtTick(measures: readonly Measure[], tick: number): number {
  let lo = 0
