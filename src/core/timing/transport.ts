...
export class Transport {
  ...
  setLoop(range: LoopRange | null): void {
    ...
    this.loopRange = {
      startTick: asTicks(range.startTick),
      endTick: asTicks(Math.min(this.finalTick, range.endTick)),
    }
    this.iteration = 0
  }
  ...
  private advance(nowMs: number, out: TransportEvent[]): void {
    for (;;) {
      const target = msToTick(this.tempo, asMillis(nowMs - this.originMs))
      ...
      if (barrier !== null) {
        ...
        // pump until the barrier is cleared — costs no musical time.
        this.reanchor()
        return
      }
      if (target < boundary) {
  ...
  private barrierStop(target: number, loop: LoopRange | null): number | null {
    ...
    const limit = loop === null ? this.finalTick + 1 : loop.endTick
    return barrier < limit ? barrier : null
  }
